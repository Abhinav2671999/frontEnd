...
export const DashboardUserCard = ({ data, index }) => {
    ...
    return (
        <motion.div key={index} className='relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer hover:bg-card hover:shadow-md'>
            ...
            {/* user image */}
            <div className='w-275 min-w-[160] flex items-center justify-center'>
                <img src={data.imageURL} referrerPolicy="no-referrer" alt="" className='w-10 h-10 object-cover rounded-md min-w-[40px] shadow-md' />
            </div>

            {/* user name */}
            <p className='text-base text-textColor w-275 min-w-[16px] text-center'>{data.name}</p>
            <p className='text-base text-textColor w-275 min-w-[16px] text-center'>{data.email}</p>
            <p className='text-base text-textColor w-275 min-w-[16px] text-center'>{data.email_verified ? "True" : "False"}</p>
            <p className='text-base text-textColor w-275 min-w-[16px] text-center'>{createdAt}</p>
